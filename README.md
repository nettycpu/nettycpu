<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=245&color=0:050505,38:111111,68:C8A96A,100:F4E8D0&text=Netty%20CPU&fontColor=FFFFFF&fontSize=58&fontAlignY=36&desc=AI%20Engineer%20%C2%B7%20Agentic%20OS%20Builder%20%C2%B7%20Creative%20Technologist&descAlignY=58&descSize=18" />

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=23&duration=2800&pause=900&color=C8A96A&center=true&vCenter=true&width=950&lines=Hello+humans%2C+I'm+Netty+%F0%9F%8F%B4%E2%80%8D%E2%98%A0%EF%B8%8F;I+build+AI+Agents+that+live+inside+the+terminal;Agentic+OS+%C2%B7+Computer+Use+Agents+%C2%B7+Automation;Code.+Design.+Automate.+Ship.+Repeat.)](https://git.io/typing-svg)

</div>

<div align="center">

<img src="https://komarev.com/ghpvc/?username=nettycpu&style=for-the-badge&color=050505&label=PROFILE%20VIEWS" />
<img src="https://img.shields.io/github/followers/nettycpu?style=for-the-badge&logo=github&logoColor=white&label=FOLLOWERS&color=C8A96A&labelColor=050505" />
<img src="https://img.shields.io/github/stars/nettycpu?style=for-the-badge&logo=github&logoColor=white&label=STARS&color=F4E8D0&labelColor=050505" />

</div>

<br />

<div align="center">

[![Website](https://img.shields.io/badge/Website-netty.dev-F4E8D0?style=for-the-badge&logo=google-chrome&logoColor=050505&labelColor=050505)](https://netty.dev)
[![GitHub](https://img.shields.io/badge/GitHub-nettycpu-050505?style=for-the-badge&logo=github&logoColor=white)](https://github.com/nettycpu)
[![Agentic OS](https://img.shields.io/badge/Building-Agentic%20OS-C8A96A?style=for-the-badge&logo=probot&logoColor=050505&labelColor=050505)](https://github.com/nettycpu/nullain-agent)

</div>

<br />

<div align="center">

[![Instagram](https://img.shields.io/badge/Instagram-netty.dev-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/netty.dev)
[![Threads](https://img.shields.io/badge/Threads-netty.dev.oficial-050505?style=for-the-badge&logo=threads&logoColor=white)](https://www.threads.com/@netty.dev.oficial?hl=pt-br)
[![YouTube](https://img.shields.io/badge/YouTube-netty--dev--oficial-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@netty-dev-oficial)
[![Spotify](https://img.shields.io/badge/Spotify-Netty-1DB954?style=for-the-badge&logo=spotify&logoColor=white)](https://open.spotify.com/intl-pt/artist/71NgkNWzJz8C6IBUNPIW1H)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-netty--dev-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/netty-dev/?skipRedirect=true)
[![Facebook](https://img.shields.io/badge/Facebook-Netty-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/profile.php?id=100090171977227&sk=directory_personal_details)

</div>

---

## 🧠 About me

<table>
<tr>
<td width="62%">

### Hello humans, my name is Netty. 🏴‍☠️☕

I'm an **AI Engineer**, **Agentic OS Builder** and **Creative Technologist** focused on building systems that combine:

- 🤖 Artificial Intelligence  
- ⚙️ Software Engineering  
- 🧩 Automation  
- 🎨 Design and Creative Production  
- 📈 Growth, marketing and product strategy  

My current mission is to build **terminal-native AI agents**, **computer use agents**, and autonomous workflows that help humans research, code, create, deploy and improve faster.

I like building tools that feel less like simple apps and more like **operating layers for intelligent work**.

</td>
<td width="38%">

### 🏴‍☠️ Netty CPU

```txt
Role        AI Engineer
Focus       Agentic OS
Mode        Terminal-first
Style       Premium hacker lab
Stack       AI · Code · Design · Growth
Mission     Build systems that execute
```

**Currently building:**  
`AI Agents` · `CLI Tools` · `Automation` · `Creative Systems`

</td>
</tr>
</table>

---

## 🧬 My Builder Identity

<div align="center">

<table>
<tr>
<td align="center" width="25%">
<h3>🧠 AI</h3>
<p>Agents, RAG, automation, prompt systems and AI workflows.</p>
</td>
<td align="center" width="25%">
<h3>⚙️ Code</h3>
<p>Backend, frontend, CLI tools, infra and open-source projects.</p>
</td>
<td align="center" width="25%">
<h3>🎨 Creative</h3>
<p>Design, video, 3D, music, visual systems and storytelling.</p>
</td>
<td align="center" width="25%">
<h3>📈 Growth</h3>
<p>Copywriting, funnels, content, marketing and product strategy.</p>
</td>
</tr>
</table>

</div>

---

## 🚀 Featured Project

<table>
<tr>
<td width="100%">

<div align="center">

### 🦊 [NULLAIN — Agentic OS / Terminal AI Agent](https://github.com/nettycpu/nullain-agent)

**Autonomous AI agent that lives inside your terminal.**

</div>

NULLAIN is my main featured project: a terminal-first agentic system designed to execute tasks, interact with the environment, connect tools, and become the foundation for a personal **Agentic OS**.

It represents the direction I am building toward: AI agents that can reason, operate, automate, document, improve workflows and become a real execution layer for creators, developers and businesses.

<div align="center">

<img src="https://img.shields.io/badge/Status-Active%20Development-C8A96A?style=for-the-badge&logo=rocket&logoColor=050505&labelColor=050505" />
<img src="https://img.shields.io/badge/Type-Terminal%20AI%20Agent-050505?style=for-the-badge&logo=gnubash&logoColor=C8A96A" />
<img src="https://img.shields.io/badge/Focus-Agentic%20OS-F4E8D0?style=for-the-badge&logo=probot&logoColor=050505&labelColor=050505" />

</div>

<br />

**Core ideas:**  
`AI Agents` · `CLI` · `Automation` · `Computer Use` · `Agentic OS` · `Tool Orchestration` · `Terminal-first Workflow`

<div align="center">

[![Open NULLAIN Repository](https://img.shields.io/badge/Open%20NULLAIN%20Repository-C8A96A?style=for-the-badge&logo=github&logoColor=050505&labelColor=050505)](https://github.com/nettycpu/nullain-agent)

</div>

</td>
</tr>
</table>

---

## 🎯 Skill Matrix

<div align="center">

<img src="https://img.shields.io/badge/AI%20Agents-050505?style=for-the-badge&logo=googlegemini&logoColor=C8A96A" />
<img src="https://img.shields.io/badge/Prompt%20Engineering-C8A96A?style=for-the-badge&logo=openai&logoColor=050505" />
<img src="https://img.shields.io/badge/RAG%20Systems-F4E8D0?style=for-the-badge&logo=databricks&logoColor=050505" />
<img src="https://img.shields.io/badge/Automation-111111?style=for-the-badge&logo=probot&logoColor=C8A96A" />
<img src="https://img.shields.io/badge/Creative%20Tech-7A5C3E?style=for-the-badge&logo=adobecreativecloud&logoColor=white" />

</div>

<br />

<table>
<tr>
<td width="50%">

### 🤖 AI / Agentic Systems

- Advanced prompt engineering  
- Custom GPTs, Gems and specialized agents  
- RAG systems with private knowledge bases  
- NotebookLM and Perplexity research workflows  
- Multi-agent orchestration for complex tasks  
- AI-assisted development with Cursor, Cline and Claude Code  
- Computer Use Agents and terminal-native agents  
- OpenAI, Gemini and local model experimentation  

</td>
<td width="50%">

### ⚙️ Development / Vibe Coding

- VS Code and Cursor as main IDE workflow  
- Git and GitHub version control  
- Terminal and CLI-first development  
- Python, TypeScript, JavaScript and Bash  
- HTML, CSS and vanilla JS fundamentals  
- Lovable, v0 and Bolt for fast prototyping  
- Ruby on Rails, Hotwire, Tailwind and PostgreSQL  
- API integrations, automation scripts and internal tools  

</td>
</tr>
<tr>
<td width="50%">

### 🧱 Backend / Infra / Deploy

- Ruby on Rails 8 ecosystem  
- PostgreSQL self-hosted  
- Docker containers  
- Kamal 2 deployment workflow  
- Hetzner and DigitalOcean VPS  
- Cloudflare CDN, DNS and protection  
- Solid Queue, Solid Cable and Solid Cache  
- GitHub Actions and deployment pipelines  

</td>
<td width="50%">

### 🎨 Design / UI / Visual Systems

- Figma for UI, UX and wireframes  
- Illustrator for logos and vector design  
- Photoshop for image editing and composition  
- Canva for fast visual assets and mockups  
- Design systems and interface direction  
- Landing pages and product visuals  
- Visual identity for brands, products and creative projects  

</td>
</tr>
<tr>
<td width="50%">

### 🎬 Video / Motion / Media

- DaVinci Resolve for editing and color grading  
- Premiere Pro for video editing  
- After Effects for motion graphics  
- CapCut for fast social media edits  
- VSL structure and audiovisual storytelling  
- Generative AI image and video workflows  
- Creative direction for short-form content  

</td>
<td width="50%">

### 🎧 Audio / Music

- FL Studio production workflow  
- Beatmaking, mixing and mastering  
- Sound design for creative projects  
- Audio identity for content, brands and products  
- Music production for digital projects  
- Sonic branding and atmosphere creation  

</td>
</tr>
<tr>
<td width="50%">

### 🧵 3D / Fashion / Creative Tech

- Blender for modeling, rendering and animation  
- CLO3D for 3D clothing simulation  
- Marvelous Designer for fabric simulation  
- Illustrator for tech packs and flats  
- 3D assets for visual projects  
- Worldbuilding and visual lore for creative universes  

</td>
<td width="50%">

### 📈 Marketing / Copy / Business

- Copywriting for sales, VSLs and landing pages  
- Social media strategy for Instagram and TikTok  
- SEO content planning  
- Paid traffic with Meta Ads and Google Ads  
- Email marketing, funnels and automation  
- Freelancing, pricing, proposals and client delivery  
- Personal brand positioning  

</td>
</tr>
</table>

---

## 🛠️ Main Stack

<div align="center">

### Core Engineering

<img src="https://skillicons.dev/icons?i=python,ts,js,ruby,rails,html,css,bash" />

### Frontend / UI

<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,figma,ai,ps" />

### Backend / Infra

<img src="https://skillicons.dev/icons?i=postgres,docker,linux,githubactions,cloudflare,vercel" />

### Tools

<img src="https://skillicons.dev/icons?i=git,github,vscode,linux,nodejs,npm" />

</div>

<br />

<div align="center">

<img src="https://img.shields.io/badge/Cursor-050505?style=for-the-badge&logo=cursor&logoColor=white" />
<img src="https://img.shields.io/badge/Claude%20Code-C8A96A?style=for-the-badge&logo=anthropic&logoColor=050505" />
<img src="https://img.shields.io/badge/OpenAI-050505?style=for-the-badge&logo=openai&logoColor=white" />
<img src="https://img.shields.io/badge/Gemini-F4E8D0?style=for-the-badge&logo=googlegemini&logoColor=050505" />
<img src="https://img.shields.io/badge/Perplexity-7A5C3E?style=for-the-badge&logo=perplexity&logoColor=white" />

</div>

<br />

<div align="center">

<img src="https://img.shields.io/badge/DaVinci%20Resolve-050505?style=for-the-badge&logo=davinciresolve&logoColor=C8A96A" />
<img src="https://img.shields.io/badge/Adobe%20Premiere-C8A96A?style=for-the-badge&logo=adobepremierepro&logoColor=050505" />
<img src="https://img.shields.io/badge/After%20Effects-F4E8D0?style=for-the-badge&logo=adobeaftereffects&logoColor=050505" />
<img src="https://img.shields.io/badge/Blender-7A5C3E?style=for-the-badge&logo=blender&logoColor=white" />
<img src="https://img.shields.io/badge/FL%20Studio-050505?style=for-the-badge&logo=musicbrainz&logoColor=C8A96A" />

</div>

---

## 🧩 Current Focus

<table>
<tr>
<td width="50%">

### 🧠 Agentic OS

Building a personal operating layer for autonomous AI agents.

- Terminal-native workflows  
- Tool-using agents  
- Computer use systems  
- Autonomous task execution  
- Memory, planning and tool orchestration  

</td>
<td width="50%">

### 🛠️ Open Source Engineering

Creating useful, inspectable and extensible tools.

- CLI-first products  
- Local-first automations  
- Secure workflows  
- Documentation-first development  
- Practical developer experience  

</td>
</tr>
<tr>
<td width="50%">

### 🎨 Creative Systems

Combining engineering with visual and media production.

- AI image and video workflows  
- Design systems  
- 3D and fashion experiments  
- Music and sound design  
- Storytelling and worldbuilding  

</td>
<td width="50%">

### 📈 Product / Growth

Turning ideas into products people can understand and use.

- Landing pages  
- Copywriting  
- Funnels  
- Content strategy  
- Personal brand systems  

</td>
</tr>
</table>

---

## 🧪 Workflow

<div align="center">

<table>
<tr>
<td align="center">🔎<br/><b>Research</b></td>
<td align="center">🧠<br/><b>Strategy</b></td>
<td align="center">⚡<br/><b>Prototype</b></td>
<td align="center">⚙️<br/><b>Build</b></td>
<td align="center">🚀<br/><b>Deploy</b></td>
<td align="center">📚<br/><b>Document</b></td>
<td align="center">🔁<br/><b>Improve</b></td>
</tr>
</table>

</div>

---

## 📊 GitHub Analytics

<div align="center">

<table>
<tr>
<td align="center" width="25%">
<h3>🏗️ Builder Mode</h3>
<p><b>Open Source</b><br/>Projects, experiments and systems.</p>
</td>
<td align="center" width="25%">
<h3>🤖 Agentic Focus</h3>
<p><b>AI Agents</b><br/>Terminal-native autonomous workflows.</p>
</td>
<td align="center" width="25%">
<h3>🎨 Creative Tech</h3>
<p><b>Design + Media</b><br/>Visual systems, video, 3D and music.</p>
</td>
<td align="center" width="25%">
<h3>⚡ Ship Energy</h3>
<p><b>Build Fast</b><br/>Prototype, deploy, document and improve.</p>
</td>
</tr>
</table>

</div>

<br />

<div align="center">

<img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=nettycpu&theme=github-compact&hide_border=true&area=true&custom_title=Netty's%20Contribution%20Graph&color=C8A96A&line=C8A96A&point=F4E8D0&area_color=7A5C3E&title_color=F4E8D0&bg_color=050505" />

</div>

<br />

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=nettycpu&theme=onedark&no-frame=true&no-bg=true&margin-w=8&margin-h=8" />

</div>

---

## 🏴‍☠️ Philosophy

<div align="center">

### I don't just want to code tools.  
### I want to build systems that think, execute, adapt and evolve.

</div>

I believe the next generation of software will not be only apps, dashboards or scripts.

It will be composed of **agents**, **tools**, **workflows**, **interfaces**, **memory systems** and **autonomous execution layers** that extend human capability.

That is the kind of software I am building.

---

## 🌐 Connect

<div align="center">

[![Website](https://img.shields.io/badge/Website-netty.dev-F4E8D0?style=for-the-badge&logo=google-chrome&logoColor=050505&labelColor=050505)](https://netty.dev)
[![GitHub](https://img.shields.io/badge/GitHub-nettycpu-050505?style=for-the-badge&logo=github&logoColor=white)](https://github.com/nettycpu)
[![Projects](https://img.shields.io/badge/Projects-Open%20Source-C8A96A?style=for-the-badge&logo=githubsponsors&logoColor=050505&labelColor=050505)](https://github.com/nettycpu?tab=repositories)

</div>

<br />

<div align="center">

[![Instagram](https://img.shields.io/badge/Instagram-netty.dev-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/netty.dev)
[![Threads](https://img.shields.io/badge/Threads-netty.dev.oficial-050505?style=for-the-badge&logo=threads&logoColor=white)](https://www.threads.com/@netty.dev.oficial?hl=pt-br)
[![YouTube](https://img.shields.io/badge/YouTube-netty--dev--oficial-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@netty-dev-oficial)

</div>

<div align="center">

[![Spotify](https://img.shields.io/badge/Spotify-Netty-1DB954?style=for-the-badge&logo=spotify&logoColor=white)](https://open.spotify.com/intl-pt/artist/71NgkNWzJz8C6IBUNPIW1H)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-netty--dev-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/netty-dev/?skipRedirect=true)
[![Facebook](https://img.shields.io/badge/Facebook-Netty-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/profile.php?id=100090171977227&sk=directory_personal_details)

</div>

<br />

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=150&color=0:F4E8D0,35:C8A96A,70:111111,100:050505&section=footer" />

</div>
